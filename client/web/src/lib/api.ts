...
export async function closeSellerBidding(token: string, vehicleId: string, forceClose = false, winningBidId?: string | null) {
  return request<{
    vehicle_id: string;
    ...
    method: "POST",
    headers: { Authorization: `Bearer ${token}` },
    body: JSON.stringify({ force_close: forceClose, winning_bid_id: winningBidId ?? null }),
  });
}
